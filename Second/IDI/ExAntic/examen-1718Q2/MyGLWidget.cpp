...
void MyGLWidget::paintGL ()
{
  ...
  modelTransformModel (2.0,0);
  glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);

  modelTransformModel (4.0,-1*float(M_PI/2.0));
  glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);

  modelTransformModel (6.0,float(M_PI/2.0));
  glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);

  modelTransformModel (8.0,float(M_PI));
  glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);
...
void MyGLWidget::modelTransformModel (float escala,float angleRotate)
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::rotate(TG, angleRotate,glm::vec3(0,1,0));
  TG = glm::translate(TG,glm::vec3(8,0,8));
  TG = glm::rotate(TG, -1*float(M_PI/2.0 + M_PI/4.0),glm::vec3(0,1,0));
  TG = glm::scale(TG, glm::vec3(escala*escalaModel,escala*escalaModel,escala*escalaModel));
...
void MyGLWidget::createBuffersTerra ()
{
  ...
  glm::vec3 amb(0,0.4,0.0);
  glm::vec3 diff(0,0.6,0.0);
