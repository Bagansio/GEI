...
void MyGLWidget::valorsInicials()
{
  ...
  centreEngraA    = glm::vec3(-0.3,0.2,0);
  rEngraA         = .35;
  angleEngraA     = 0;
  
  centreEngraB    = glm::vec3(0.4,-0.2,0);
  rEngraB         = .25;
  angleEngraB     = 0;
  ...
  relacioPEA = -gir * rPinyo / rEngraA;
  relacioPEB = -gir * rPinyo / rEngraB;
}
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_M:
        anglePinyo += gir;
        angleEngraA += relacioPEA;//= -anglePinyo * relacioPEA;//rPinyo / rEngraA;
        angleEngraB += relacioPEB;//= -anglePinyo * relacioPEB;//rPinyo / rEngraB;
        break;
