...
void muerte_hijo(int s)
{
	if (s == SIGCHLD)
	{
		...
		while(pid > 0)
		{
			...
			if (WIFEXITED(res))
			{
				sprintf(buff,"Muere el proceso %d\n",pid);
			}
	        else
			{
				if(WTERMSIG(res) == SIGUSR1) sprintf(buff,"Muere el proceso %d por el SIGUSR1\n",pid);
				else sprintf(buff,"Muere el proceso %d por el signal %d\n",pid, WTERMSIG(res));
			}
