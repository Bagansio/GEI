...
void Cjt_estudiants::afegir_estudiant(const Estudiant& est) {
 ...
 if (est.te_nota()) {
  ...
  else if (est.consultar_DNI() < vest[imax].consultar_DNI()) ++imax;
}
...
void Cjt_estudiants::modificar_estudiant(const Estudiant& est) {
  ...
  vest[i] = est;
  if (imax == i) recalcular_posicio_imax();
  else if (est.te_nota()) {
...
void Cjt_estudiants::esborrar_estudiant(int dni)
{
  int i=cerca_dicot(vest,0,nest-1,dni);
  int max = i;
  if (i==-1) throw PRO2Excepcio("No existeix un estudiant amb el dni especificat");
  while (i<nest-1) {
    vest[i]=vest[i+1];
    ++i;
  }
  --nest;
  if(max == imax) recalcular_posicio_imax();
  else if (max < imax) --imax;
}

Estudiant Cjt_estudiants::estudiant_nota_max() const
{
  if (imax == -1) throw PRO2Excepcio("No existeix un estudiant amb nota maxima");
  return vest[imax];
}