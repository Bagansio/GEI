  ...
  if (not b)
    {
      ...
      vest[i] = est;
      ++nest;
      if (est.te_nota() and est.consultar_nota() < 5)
      {
        ++nombre_suspesos;
        if (est.consultar_nota() >= 4) ++nombre_compensables;
      }
      else if (not est.te_nota()) ++nombre_suspesos;
    }
    }
     ...
     del par�metre impl�cit */
{
  ...
  if (b) {
    ...
    // actualitzem la mida
    if (vest[i].te_nota() and vest[i].consultar_nota() < 5)
    {
        --nombre_suspesos;
        if (vest[i].consultar_nota() >= 4) --nombre_compensables;
    }
    else if (not vest[i].te_nota()) --nombre_suspesos;
